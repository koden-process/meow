  ...
  return (
    <>
      <div className={`item ${isActiveRoute('/') ? 'active' : ''}`}>
        <Link to="/" title={Translations.OpportunityTab[DEFAULT_LANGUAGE]}>
          <img alt={Translations.OpportunityTab[DEFAULT_LANGUAGE]} src={`/${currency?.toLocaleLowerCase()}-icon.svg`} />
        </Link>
      </div>
